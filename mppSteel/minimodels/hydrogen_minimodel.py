...
from mppSteel.hydrogen_assumptions import (
    ...
    ENERGY_CONSUMPTION_END,
    CAPACITY_UTILIZATION_FACTOR,
    FIXED_OPEX,
    LEVELIZED_H2_STORAGE_COST_FAVORABLE,
    LEVELIZED_H2_STORAGE_COST_AVERAGE,
    HYDROGEN_LHV,
...
def timeseries_generator(
    timeseries_type: str, start_year: int, end_year: int, units: str = "", **kwargs
) -> pd.DataFrame:
    ...
    def generic_value_logic(
        ...
        for row in df_c.itertuples():
            ...
            # middle years
                ...
                df_c.loc[row.Index, "value"] = mid_values_function(
                    start_price, end_price, previous_value
                )
            # last year
            else:
    ...
    if timeseries_type == "vre_price":
        df = generic_value_logic(
            ...
            value_start_price=vre_price_selector(
                hydrogen_scenario, HYDROGEN_PRICE_START_YEAR
            )
            * EUR_USD_CONVERSION
            / 1000,
            value_end_price=vre_price_selector(
                hydrogen_scenario, HYDROGEN_PRICE_END_YEAR
            )
            * EUR_USD_CONVERSION
            / 1000,
    ...
    if timeseries_type == "stack_lifetime":
        df = generic_value_logic(
            df=df,
            value_start_price=STACK_LIFETIME_START.value
            / 365
            / 24
            / CAPACITY_UTILIZATION_FACTOR.value,
            value_end_price=STACK_LIFETIME_END.value
            / 365
            / 24
            / CAPACITY_UTILIZATION_FACTOR.value,
...
def create_green_h2_prices(
    df: pd.DataFrame, scenario: str, as_gj: bool = False
) -> pd.DataFrame:
    ...
    for row in df_c.itertuples():
        ...
        capex = (
            row.electrolyzer_capex * row.energy_consumption * CAPITAL_RECOVERY_FACTOR
        )
        opex = row.electrolyzer_capex * row.energy_consumption * FIXED_OPEX.value
        replacements = (
            row.required_stack_replacements
            * row.stack_capex
            * row.energy_consumption
            / ELECTROLYZER_LIFETIME.value
        )
        utilization = 365 * 24 * CAPACITY_UTILIZATION_FACTOR.value
        storage_costs = storage_price * EUR_USD_CONVERSION
        value = (
            energy_cons + ((capex + opex + replacements) / utilization) + storage_costs
        )
...
def generate_hydrogen_timeseries(serialize_only: bool = False) -> pd.DataFrame:
    ...
    df_grid_reference = create_df_grid(
        [
            vre_price_average,
            vre_price_favorable,
            electrolyzer_capex_timeseries,
            stack_lifetime,
            required_stack_replacements,
            stack_capex,
            energy_consumption,
        ]
    )
    ...
    green_h2_prices_average_gj = create_green_h2_prices(
        df_grid_reference, "average", as_gj=True
    )
    green_h2_prices_favorable_gj = create_green_h2_prices(
        df_grid_reference, "favorable", as_gj=True
    )
    hydrogen_minimodel_timeseries = pd.concat(
        [green_h2_prices_favorable_gj, green_h2_prices_average_gj]
    )
