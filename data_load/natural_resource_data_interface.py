...
def solar_formatter(df: pd.DataFrame, scaled: bool = False) -> pd.DataFrame:
    ...
    economic_columns_of_interest = [
        'iso_a3', 'country_or_region', 'total_population_2018', 'total_area_2018', 'human_development_index_2017',
        'gross_domestic_product_(usd_per_capita)_2018']

    solar_columns_of_interest = [
        'iso_a3', 'country_or_region',
...
def solar_getter(df: pd.DataFrame, country_code: str, metric: str = ''):
    ...
    df_c = df.copy()
    if metric:
        logger.info(f'Getting the value for Solar | Country: {country_code} - Metric: {metric}')
...
def wind_getter(df: pd.DataFrame, country_code: str, metric: str = ''):
    ...
    df_c = df.copy()
    if metric:
        logger.info(f'Getting the value for Wind | Country: {country_code} - Metric: {metric}')
