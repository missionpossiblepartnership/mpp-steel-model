...
def business_case_formatter_splitter(df: pd.DataFrame):
    logger.info('Splitting the business cases into two DataFrames: Parameters and Processes')
    df_c = df.copy()
...
def get_all_steam_values(df: pd.DataFrame, technology: str, process_list: list, factor_mapper: dict):
...
def get_all_electricity_values(
    df: pd.DataFrame, technology: str, process_list: list, factor_mapper: dict = [], as_dict: bool = False):
...
def create_production_factors(technology: str, furnace_group_dict: dict, hard_coded_factors: dict):
    ...
    if technology in bosc_factor_group:
        ...
        if technology in furnace_group_dict['dri-bof']:
            ...
            shaft_furnace_pellets = tech_process_getter(bc_processes, technology, 'Shaft Furnace', step='Pellets') * SHAFT_FURNACE_FACTOR
...
def limestone_df_editor(
    ...
    if technology in electricity_self_gen_group:
        ...
        if technology in furnace_group_dict['blast_furnace']:
            ...
            bof_lime = tech_process_getter(bc_processes, technology, 'Limestone', step='BOF lime') * factor_dict['Basic Oxygen Steelmaking + Casting']
            limestone_df.loc[limestone_df['step'] == 'BOF lime', 'value'] = bof_lime
            blast_furnace_lime = tech_process_getter(bc_processes, technology, 'Limestone', step='Blast furnace lime')
            limestone_df.loc[limestone_df['step'] == 'Blast furnace lime', 'value'] = blast_furnace_lime * factor_dict['Blast Furnace']
...
def ccs_df_editor(
    ...
    if technology in furnace_group_dict['ccs']:
        ...
        if technology in ['Smelting Reduction+CCUS']:
            ...
            natural_gas_ccs = 0
            smelting_furnace_thermal_coal = tech_process_getter(df_dict_c['Smelting Furnace'], technology, process='Smelting Furnace', material='Thermal coal')
            thermal_coal_ef = ef_dict['Thermal coal']
            smelting_furnace_natural_gas = tech_process_getter(df_dict_c['Smelting Furnace'], technology, process='Smelting Furnace', material='Natural gas')
            CCS_FACTOR = (smelting_furnace_thermal_coal * thermal_coal_ef / 1000) + ((natural_gas_ccs + smelting_furnace_natural_gas) * EF_DICT['Natural gas'] / 1000) + ((1-electricity_share_factor) * 10 * (EF_DICT['BOF gas'] / 1000))

        if technology in ['DRI-EAF+CCUS']:
            pellets_natural_gas = tech_process_getter(df_dict_c['Pelletisation'], technology, process='Pelletisation', material='Natural gas')
            shaft_furnace_natural_gas = tech_process_getter(df_dict_c['Shaft Furnace'], technology, process='Shaft Furnace', material='Natural gas')
            eaf_natural_gas = tech_process_getter(df_dict_c['EAF (Steel-making) + Casting'], technology, process='EAF (Steel-making) + Casting', material='Natural gas')
            eaf_process_emissions = tech_process_getter(df_dict_c['EAF (Steel-making) + Casting'], technology, process='EAF (Steel-making) + Casting', material='Process emissions')
            CCS_FACTOR = ((pellets_natural_gas + pellets_natural_gas + eaf_natural_gas) * (ef_dict['Natural gas'] / 1000)) + eaf_process_emissions

        if technology in ['DRI-Melt-BOF+CCUS']:
            selected_processes_df = concat_process_dfs(df_dict_c, ['Shaft Furnace', 'Remelt'])
        ...
        if technology in ['BAT BF-BOF+CCUS', 'BAT BF-BOF+BECCUS']:
            electricity_share_factor = tech_parameter_getter(bc_parameters, technology, 'Share of electricity purchased in total demand')
            limestone_process_emissions = tech_process_getter(df_dict_c['Limestone'], technology, process='Limestone', step='Process emissions')
            selected_processes_df = concat_process_dfs(df_dict_c, ['Coke Production', 'Sintering', 'Pelletisation', 'Blast Furnace'])
            ...
            CCS_FACTOR = (ef_sum_product_large / 1000) + ((ef_sum_product_small * ef_dict['Electricity']) / 1000) + 0.52 * (1 - electricity_share_factor) + limestone_process_emissions

        # captured co2 value
        captured_co2 = tech_process_getter(df_dict_c['CCS'], technology, process='CCS', step='Captured CO2')
        captured_co2_factored = captured_co2 * CCS_FACTOR
        ccs_df.loc[(ccs_df['step'] == 'Captured CO2'), 'value'] = captured_co2_factored
        compression_electricity = tech_process_getter(df_dict_c['CCS'], technology, process='CCS', process_detail='Compression')
        ...
        if technology in ['DRI-EAF+CCUS']:
            ccs_df.loc[(ccs_df['material_category'] == 'Natural gas'), 'value'] = reboiler_duty_natural_gas * CCS_FACTOR
        ...
        if technology in ['DRI-Melt-BOF+CCUS', 'BAT BF-BOF+BECCUS']:
            column_matcher_dict = {
                ...
                'BAT BF-BOF+BECCUS': ['Coke Production', 'Sintering', 'Pelletisation', 'Blast Furnace']}
            selected_processes_df = concat_process_dfs(df_dict_c, column_matcher_dict[technology])
            ef_sum_product = sum_product_ef(selected_processes_df, ef_dict)
            ccs_df.loc[(ccs_df['material_category'] == 'Natural gas'), 'value'] = 3.6 * ef_sum_product / 1000
...
def ccu_df_editor(df_dict: dict, technology: str, furnace_group_dict: dict, ef_dict: dict):
    ...
    if technology in furnace_group_dict['ccu']:
        ...
        used_co2 = tech_process_getter(df_dict_c['CCU -CO-based'], technology, process='CCU -CO-based', material='Used CO2')
        ccu_co.loc[(ccu_co['material_category'] == 'Used CO2'), 'value'] = used_co2 * CCU_CO_FACTOR
        ...
        ef_sum_product_large = sum_product_ef(selected_processes_df, ef_dict)
        limestone_process_emissions = tech_process_getter(df_dict_c['Limestone'], technology, process='Limestone', step='Process emissions')
        CCU_CO2_FACTOR = (ef_sum_product_large / 1000) + limestone_process_emissions - used_co2
        selected_processes_df = concat_process_dfs(df_dict_c, ['Blast Furnace'])
        ef_sum_product_small = sum_product_ef(selected_processes_df, ef_dict, ['Electricity'])
        ccu_co2.loc[(ccu_co2['process_detail'] == 'Reboiler duty'), 'value'] = used_co2 * ef_sum_product_small / 1000
        compression_electricity = tech_process_getter(df_dict_c['CCU -CO2-based'], technology, process='CCU -CO2-based', process_detail='Compression')
        ccu_co2.loc[(ccu_co2['process_detail'] == 'Compression'), 'value'] = CCU_CO2_FACTOR * compression_electricity
...
def self_gen_df_editor(df_dict: dict, technology: str, furnace_group_dict: dict, factor_dict: dict, tech_processes_dict: dict):
    ...
    if technology in electricity_self_gen_group:
        ...
        if technology in furnace_group_dict['blast_furnace']:
            ...
            self_gen_df.loc[(self_gen_df['material_category'] == 'BF gas'), 'value'] = sum(all_electricity_values) * (1 - electricity_share_factor) * bf_gas
...
def full_model_flow(tech_name: str):
    ...
    reformated_dict_c = fix_exceptions(reformated_dict_c, tech_name, FURNACE_GROUP_DICT, process_prod_factor_mapper, TECHNOLOGY_PROCESSES)
    reformated_dict_c = limestone_df_editor(reformated_dict_c, tech_name, FURNACE_GROUP_DICT, process_prod_factor_mapper)
...
def fix_exceptions(df_dict: dict, technology: str, furnace_group_dict: dict, factor_dict: dict, process_dict: dict):
    ...
    if technology in ['Smelting Reduction']:
        ...
        electricity_share_factor = tech_parameter_getter(bc_parameters, technology, 'Share of electricity purchased in total demand')
        smelting_electricity = tech_process_getter(bc_processes, technology, process='Smelting Furnace', material='Electricity')
        smelting_furnace_df.loc[smelting_furnace_df['material_category'] == 'Electricity', 'value'] = smelting_electricity * factor_dict['Smelting Furnace'] * electricity_share_factor
    ...
    if technology in furnace_group_dict['blast_furnace']:
        ...
        # Electricity
        elec_values_dict = get_all_electricity_values(bc_processes, technology, process_dict[technology], factor_mapper=factor_dict, as_dict=True)
        ...
        for process in df_dict_c.keys():
            ...
            if 'Electricity' in temp_process_df['material_category'].unique():
                if process == 'Basic Oxygen Steelmaking + Casting':
                    temp_process_df.loc[temp_process_df['process_detail'] == 'Energy-oxygen furnace', 'value'] = elec_values_dict['Basic Oxygen Steelmaking + Casting - Oxygen']
                    temp_process_df.loc[temp_process_df['process_detail'] == 'Energy-casting', 'value'] = elec_values_dict['Basic Oxygen Steelmaking + Casting - Casting']
                    df_dict_c[process] = temp_process_df
                else:
                    temp_process_df.loc[temp_process_df['process_detail'] == process, 'value'] = elec_values_dict[process]
                    df_dict_c[process] = temp_process_df
        ...
        bat_lcv = lcv = tech_parameter_getter(bc_parameters, 'BAT BF-BOF', 'LCV of injected reductant')
        ...
        if technology in ['BAT BF-BOF+BECCUS', 'BAT BF-BOF_bio PCI']:
            ...
            blast_furnace_df.loc[(blast_furnace_df['process_detail'] == 'Tuyere injection'), 'value'] = biomass_calculation
...
electricity_self_gen_group = FURNACE_GROUP_DICT['blast_furnace'] + FURNACE_GROUP_DICT['smelting_reduction']
