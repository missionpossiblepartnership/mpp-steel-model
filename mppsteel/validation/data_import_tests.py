...
COUNTRY_REF_SCHEMA = DataFrameSchema({
    'Country': Column(str),
    'ISO-alpha3 code': Column(str, Check.str_length(3), required=True, nullable=True),
    'M49 Code': Column(int),
    'Region 1': Column(str),
...
ETHANOL_PLASTIC_CHARCOAL_SCHEMA = DataFrameSchema(
    columns={
        ...
        'Trade Flow': Column(str, Check.isin(['Import', 'Export', 'Re-Export', 'Re-Import'])),
...
def import_data_tests():
    ...
    regional_steel_demand = read_pickle_folder(PKL_DATA_IMPORTS, "regional_steel_demand")
