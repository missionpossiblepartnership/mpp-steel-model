...
from mppsteel.utility.reference_lists import MPP_COLOR_LIST
...
def format_cot_graph(df: pd.DataFrame, regions: list = None, resource_list: list = None) -> pd.DataFrame:
    df_c=df.copy()
    df_c=pd.melt(df, id_vars=['year', 'region_wsa_region'], value_vars=resource_list, var_name='metric')
    df_c.reset_index(drop=True, inplace=True)
    df_c=df_c.groupby(['region_wsa_region','year','metric'], as_index=False).agg({"value": 'sum'}).round(2)
    if regions:
        df_c =df_c.loc[df_c['region_wsa_region'].isin(regions)]
    else:
        df_c=df_c.groupby(['year','metric'],as_index=False).agg({"value": 'sum'}).round(2)
    return df_c

def consumption_over_time_graph(regions: list = None, save_filepath:str=None, ext:str='png') -> px.bar:
