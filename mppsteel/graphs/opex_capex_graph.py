...
def return_capex_values(capex_dict: dict, year: str, investment_cycle: int, discount_rate: float):
    ...
    brownfield_values['value'].apply(lambda x: npf.npv(discount_rate, np.full((investment_cycle), x)))
    greenfield_values['value'].apply(lambda x: npf.npv(discount_rate, np.full((investment_cycle), x)))
