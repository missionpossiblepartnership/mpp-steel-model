...
def add_scenarios(df: pd.DataFrame, scenario_dict: dict, single_line: bool = False) -> pd.DataFrame:
    df_c = df.copy()
    if single_line:
...
def add_regions(df: pd.DataFrame, country_ref_dict: dict, country_ref_col: str, region_schema: str) -> pd.DataFrame:
    df_c = df.copy()
    df_c[f'region_{region_schema}'] = df_c[country_ref_col].apply(
        lambda country: get_region_from_country_code(country, region_schema, country_ref_dict))
    return df_c


def add_results_metadata(
    df: pd.DataFrame, scenario_dict: dict, include_regions: bool = True, single_line: bool = False) -> pd.DataFrame:
