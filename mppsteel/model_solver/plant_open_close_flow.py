...
def open_real_plants(
    ...
    active_steel_plants_df = steel_plant_df[steel_plant_df["active_check"] == True].copy()
...
def return_len_closed_plants_closed_in_year(steel_plant_df: pd.DataFrame, year: int) -> int:
...
def close_real_plants(
    ...
    for region in list(production_demand_gap_analysis.keys()):
        ...
        initial_number_of_closed_plants = return_len_inactive_plants(updated_steel_plant_df)
        ...
        if plants_to_close > 0:
            ...
            min_capacity_to_close = -production_demand_gap_analysis[region]["new_capacity_required"]
            assert round(new_capacity, TRADE_ROUNDING_NUMBER) == round(initial_capacity - min_capacity_to_close, TRADE_ROUNDING_NUMBER), f"Region: region | target_capacity: {new_capacity} | initial_capacity: {initial_capacity} | min_capacity_to_close: {min_capacity_to_close} -> {production_demand_gap_analysis[region]}"
            production_dict_value = production_demand_gap_analysis[region]["new_utilized_capacity"]
            potential_plants_to_close = return_plants_from_region(active_steel_plants_df, region)
            initial_utilization = production_demand_gap_analysis[region]["new_utilization"]
            closed_plants_prior = return_len_closed_plants_closed_in_year(updated_steel_plant_df, year)
            ...
            capacity_removal_actual_minus_indicative = capacity_removed - min_capacity_to_close
            closed_plants_post = return_len_closed_plants_closed_in_year(updated_steel_plant_df, year)
            new_number_of_closed_plants = return_len_inactive_plants(updated_steel_plant_df)
...
def open_close_plants(
    ...
    updated_steel_plant_df = open_real_plants(
        ...
        capacity_container, capacity_constraint_container, 
    ...
    updated_steel_plant_df = close_real_plants(
        ...
        util_max, util_min, regional_scrap
    )

    test_utilization_values(utilization_container, production_demand_gap_analysis, year, util_min, util_max)
    ...
    new_active_plants = updated_steel_plant_df[updated_steel_plant_df["active_check"] == True].copy()
    ...
    investment_container.add_new_plants(
        new_open_plants["plant_name"], new_open_plants["start_of_operation"], investment_cycle_randomness
    )
    return updated_steel_plant_df
