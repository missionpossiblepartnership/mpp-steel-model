...
def create_new_capacity_subset(plant_df: pd.DataFrame, data_agg_type: str = 'sum'):
    ...
    if data_agg_type == 'sum':
        new_plants_df = new_plants_df.rename(mapper={'start_of_operation': 'year'}, axis=1)
        new_plants_df = new_plants_df[['plant_name', 'year', 'end_of_operation', 'plant_capacity', 'rmi_region']].groupby(['year', 'rmi_region']).agg('sum')
    ...
    if data_agg_type == 'cumsum':
        ...
        for region in regions:
            region_df = new_plants_df[new_plants_df["rmi_region"] == region][['start_of_operation', 'plant_capacity']].copy()
            region_df = region_df.groupby(["start_of_operation"]).sum()
            new_df = pd.DataFrame({'year': MODEL_YEAR_RANGE})
            new_df['plant_capacity'] = new_df['year'].apply(lambda year: assign_cumsum_value(region_df, year))
    ...
    new_plants_df['plant_capacity_mt'] = new_plants_df['plant_capacity'] / MEGATON_TO_KILOTON_FACTOR
