    ...
    def value_mapper(row, enum_dict):
        if materials_to_exclude:
            if (row[enum_dict["material_category"]] in ef_dict.keys()) & (
                row[enum_dict["material_category"]] not in materials_to_exclude
            ):
                row[enum_dict["material_emissions"]] = (
                    row[enum_dict["value"]]
                    * ef_dict[row[enum_dict["material_category"]]]
                )
            else:
...
def get_all_steam_values(
    ...
    for process in process_list:
        if (
            "Steam" in df_c[df_c["process"] == process]["material_category"].unique()
        ):  # changed
    ...
    if technology in bosc_factor_group:
        ...
        if technology in furnace_group_dict["dri-bof"]:
            ...
            shaft_furnace_pellets = (
                tech_process_getter(
                    bc_processes, technology, "Shaft Furnace", material="Iron ore"
                )
                * SHAFT_FURNACE_FACTOR
            )  # Iron ore, but should be pellets
    ...
    if technology in create_hardcoded_exceptions(
        ...
        if furnace_group in hard_coded_factors.keys():
            for process in hard_coded_factors[furnace_group]:
                process_factor_mapper[process] = hard_coded_factors[furnace_group][
                    process
                ]
...
def limestone_df_editor(
    ...
    if technology in electricity_self_gen_group:
        ...
        if technology in furnace_group_dict["blast_furnace"]:
            ...
            limestone_df.loc[
                limestone_df["step"] == "Blast furnace lime", "value"
            ] = blast_furnace_lime
...
def fix_ccs_factors(
    ...
    if technology in ["DRI-EAF+CCUS"]:
        ...
        factor_dict_c["CCS"] = (
            (shaft_furnace_natural_gas + pellets_natural_gas + eaf_natural_gas)
    ...
    if technology in furnace_group_dict["ccu"]:
        ...
        used_co2 = (
            tech_process_getter(
                bc_processes, technology, process="CCU -CO-based", material="Used CO2"
            )
            * ccu_factor
        )
        factor_dict_c["CCU -CO2-based"] = (
...
def fix_ccs_bat_ccus_factors(
    ...
    if technology in ["BAT BF-BOF+CCUS", "BAT BF-BOF+BECCUS"]:
        ...
        ef_sum_product_small = sum_product_ef(
            r_dict["Blast Furnace"], ef_dict_c, ["Electricity"]
        )
        s_factor = special_factor_dict[technology]
        ccs_factor = (
            (ef_sum_product_large / 1000)
            + ((ef_sum_product_small * ef_dict_c["Electricity"]) / 1000)
            + (s_factor * (1 - electricity_share_factor))
...
def ccs_df_editor(
    ...
    if technology in furnace_group_dict["ccs"]:
        ...
        captured_co2 = tech_process_getter(
            bc_processes, technology, process="CCS", step="Captured CO2"
        )
        ...
        compression_electricity = tech_process_getter(
            bc_processes, technology, process="CCS", process_detail="Compression"
        )
        ...
        if technology in ["DRI-EAF+CCUS"]:
            curr_ng_val = tech_process_getter(
                ...
                technology,
                process="CCS",
                process_detail="Reboiler duty",
                material="Natural gas",
            )
            ccs_df.loc[
                ccs_df["process_detail"].eq("Reboiler duty")
                & ccs_df["material_category"].eq("Natural gas"),
                "value",
            ] = (curr_ng_val * factor_dict["CCS"])
            ccs_df.loc[
                ccs_df["process_detail"].eq("Reboiler duty")
                & ccs_df["material_category"].eq("Natural gas"),
                "material_category",
        ...
        if technology in ["BAT BF-BOF+BECCUS", "BAT BF-BOF+CCUS"]:
            ...
            ef_dict_c["Biomass"] = 95
            ef_sum_product_small = sum_product_ef(
                df_dict_c["Blast Furnace"], ef_dict_c, ["Electricity"]
            )
            elec_reboiler = 3.6 * ef_sum_product_small / 1000
            ccs_df.loc[
                (ccs_df["material_category"] == "Natural gas"), "value"
            ] = elec_reboiler
            ccs_df.loc[
        ...
        if technology in ["DRI-Melt-BOF+CCUS"]:
            ...
            ef_sum_product = sum_product_ef(
                selected_processes_df, ef_dict, ["BOF gas", "BF gas", "COG"]
            )
...
def ccu_df_editor(
    ...
    if technology in furnace_group_dict["ccu"]:
        ...
        ccu_co.loc[(ccu_co["material_category"] == "Used CO2"), "value"] = (
            used_co2 * factor_dict["CCU -CO-based"]
        )
        ...
        ccu_co2.loc[(ccu_co2["process_detail"] == "Compression"), "value"] = (
            compression_electricity * factor_dict["CCU -CO2-based"]
        )
        captured_co2 = tech_process_getter(
        ...
        ccu_co2.loc[(ccu_co2["material_category"] == "Captured CO2"), "value"] = (
            captured_co2 * factor_dict["CCU -CO2-based"]
        )
...
def self_gen_df_editor(
    ...
    if technology in electricity_self_gen_group:
        ...
        if technology in furnace_group_dict["smelting_reduction"]:
            ...
            oxygen_elec_value = bosc_df.loc[
                bosc_df["process_detail"].eq("Energy-oxygen furnace")
                & bosc_df["material_category"].eq("Electricity"),
                "value",
            ].values[0]
            casting_value = bosc_df.loc[
                bosc_df["process_detail"].eq("Energy-casting")
                & bosc_df["material_category"].eq("Electricity"),
                "value",
            ].values[0]
            temp_elec_values.append(
                oxygen_elec_value * factor_dict["Basic Oxygen Steelmaking + Casting"]
            )
            temp_elec_values.append(
                casting_value * factor_dict["Basic Oxygen Steelmaking + Casting"]
            )
            if technology in ["Smelting Reduction+CCUS"]:
                electricity_value_general = tech_process_getter(
                    bc_processes, technology, process="CCS", material="Electricity"
                )
                cap_co2_value = tech_process_getter(
                    bc_processes, technology, process="CCS", step="Captured CO2"
                )
                val_to_append = (
                    cap_co2_value * factor_dict["CCS"] * electricity_value_general
                )
                temp_elec_values.append(val_to_append)

            self_gen_df.loc[
                (self_gen_df["material_category"] == "BOF gas"), "value"
            ] = (sum(temp_elec_values) * (1 - electricity_share_factor) * bof_gas)
        ...
        if technology in furnace_group_dict["blast_furnace"]:
            if technology in ["BAT BF-BOF+CCUS", "BAT BF-BOF+BECCUS"]:
                ...
                elec_comp_val = tech_process_getter(
                    bc_processes,
                    technology,
                    process="CCS",
                    process_detail="Compression",
                ...
                cap_co2_val = tech_process_getter(
                    bc_processes, technology, process="CCS", step="Captured CO2"
                )
            ...
            self_gen_df.loc[
                (self_gen_df["material_category"] == "COG"), "value"
            ] = cog_val
            ...
            self_gen_df.loc[
                (self_gen_df["material_category"] == "BF gas"), "value"
            ] = bf_gas_val
...
def fix_exceptions(
    ...
    # Electricity
    if (technology in furnace_group_dict["blast_furnace"]) or (
        technology in ["Smelting Reduction"]
    ):
        ...
        for process in df_dict_c.keys():
            if "Electricity" in df_dict_c[process]["material_category"].unique():
                if process == "Basic Oxygen Steelmaking + Casting":
                    ...
                    oxygen_elec_value = t_df.loc[
                        t_df["process_detail"].eq("Energy-oxygen furnace")
                        & t_df["material_category"].eq("Electricity"),
                        "value",
                    ]
                    casting_value = t_df.loc[
                        t_df["process_detail"].eq("Energy-casting")
                        & t_df["material_category"].eq("Electricity"),
                        "value",
                    ]
                    t_df.loc[
                        ...
                        & t_df["material_category"].eq("Electricity"),
                        "value",
                    ] = (oxygen_elec_value * electricity_share)
                    t_df.loc[
                        t_df["process_detail"].eq("Energy-casting")
                        & t_df["material_category"].eq("Electricity"),
                        "value",
                    ] = (casting_value * electricity_share)
                    df_dict_c[process] = t_df
                elif process == "Oxygen Generation":
                    t_df = df_dict_c[process]
                    curr_val = t_df.loc[
                        t_df["process"].eq(process)
                        & t_df["material_category"].eq("Electricity"),
                        "value",
                    ]
                    oxygen_elec_value_final = factor_dict[process]
                    if technology in ["BAT BF-BOF+BECCUS", "BAT BF-BOF+CCUS"]:
                        oxygen_elec_value_final = (
                            factor_dict[process] * electricity_share
                        )
                    t_df.loc[
                        t_df["process"].eq(process)
                        & t_df["material_category"].eq("Electricity"),
    ...
    if technology in ["Smelting Reduction+CCUS"]:
        for process in df_dict_c.keys():
            ...
            if process == "CCS":
                ...
                cap_co2_value = tech_process_getter(
                    bc_processes, technology, process=process, material="Captured CO2"
                )
                t_df.loc[
                    t_df["process"].eq(process)
                    & t_df["material_category"].eq("Electricity"),
                    "value",
                ] = (cap_co2_value * factor_dict[process] * electricity_value_general)
                df_dict_c[process] = t_df
    ...
    if technology in furnace_group_dict["dri-bof"]:
        for process in df_dict_c.keys():
            if process == "Basic Oxygen Steelmaking + Casting":
                t_df = df_dict_c[process]
                factor = factor_dict[process]
                oxygen_elec_value = tech_process_getter(
                    bc_processes,
                    technology,
                    process=process,
                    process_detail="Energy-oxygen furnace",
                ...
                casting_value = tech_process_getter(
                    bc_processes,
                    technology,
                    process=process,
                    process_detail="Energy-casting",
                ...
                t_df.loc[
                    ...
                    & t_df["material_category"].eq("Electricity"),
                    "value",
                ] = (oxygen_elec_value * factor)
                t_df.loc[
                    t_df["process_detail"].eq("Energy-casting")
                    & t_df["material_category"].eq("Electricity"),
                    "value",
                ] = (casting_value * factor)
                df_dict_c[process] = t_df
            if process == "Oxygen Generation":
                t_df = df_dict_c[process]
                curr_val = t_df.loc[
        ...
        if technology in ["DRI-Melt-BOF_100% zero-C H2"]:
            ...
            t_df.loc[
                t_df["step"].eq("Natural gas")
                & t_df["material_category"].eq("Natural gas"),
                "material_category",
            ] = "Electricity"
    ...
    if technology in furnace_group_dict["dri-eaf"]:
        for process in df_dict_c.keys():
            if process == "EAF (Steel-making) + Casting":
                ...
                pre_heating_ng = tech_process_getter(
                    bc_processes,
                    technology,
                    process=process,
                    process_detail="Pre-heating and process control",
                    material="Natural gas",
                )
                casting_value = tech_process_getter(
                    bc_processes,
                ...
                t_df.loc[
                    t_df["process_detail"].eq("Pre-heating and process control")
                    & t_df["material_category"].eq("Natural gas"),
                    "value",
                ] = (pre_heating_ng * factor)
                t_df.loc[
                    t_df["process_detail"].eq("Energy-casting")
                    & t_df["material_category"].eq("Natural gas"),
                    "value",
                ...
                if technology in ["DRI-EAF_50% bio-CH4"]:
                    t_df.loc[
                        t_df["material_category"].eq("Biomethane"), "material_category"
                    ] = "Natural gas"
    ...
    if technology in furnace_group_dict["blast_furnace"]:
        ...
        if technology in ["BAT BF-BOF+BECCUS", "BAT BF-BOF_bio PCI"]:
            ...
            blast_furnace_df.loc[
                blast_furnace_df["process_detail"].eq("Tuyere injection")
                & blast_furnace_df["material_category"].eq("Biomass"),
                "value",
            ] = biomass_calculation
        ...
        if technology in ["BAT BF-BOF_H2 PCI"]:
            ...
            hydrogen = tech_process_getter(
                bc_processes, technology, process="Blast Furnace", material="Hydrogen"
            )
...
def full_model_flow(
    ...
    if material:
        ...
        reformated_dict = create_mini_process_dfs(
            bc_processes, technology, TECHNOLOGY_PROCESSES, process_prod_factor_mapper
        )
        df_container.append(
            get_material_values(
                reformated_dict,
        ...
        reformated_dict_c = switch_ironore_and_pellets(
            reformated_dict_c
        )  # delete this line once fixed!!!!!
